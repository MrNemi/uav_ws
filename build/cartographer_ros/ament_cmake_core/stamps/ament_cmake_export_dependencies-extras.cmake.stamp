...
set(_exported_dependencies "builtin_interfaces;cartographer;cartographer_ros_msgs;geometry_msgs;nav_msgs;pcl_conversions;rclcpp;sensor_msgs;std_msgs;tf2;tf2_eigen;tf2_msgs;tf2_ros;visualization_msgs;rosbag2_cpp;rosbag2_storage;urdf;urdfdom")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to cartographer_ros_DEFINITIONS, cartographer_ros_INCLUDE_DIRS,
# cartographer_ros_LIBRARIES, and cartographer_ros_LINK_FLAGS.
# Additionally collect the direct dependency names in
# cartographer_ros_DEPENDENCIES as well as the recursive dependency names
# in cartographer_ros_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(cartographer_ros_DEPENDENCIES ${_exported_dependencies})
  set(cartographer_ros_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(cartographer_ros_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(cartographer_ros_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(cartographer_ros_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(cartographer_ros_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(cartographer_ros_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND cartographer_ros_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
